...
async def start_campaign(campaign_id: str):
    """Start a campaign - queues execution as a background asyncio task."""
    from services.orchestration.tasks_queue.tasks import execute_campaign
    from services.orchestration.tasks_queue.celery_app import submit_task
    ...
    logger.info(f"Campaign {campaign_id} queued for execution: task_id={task_id}")
    ...
    return CampaignResponse(
        ...
        status=campaign.status.value,
        total_contacts=campaign.total_contacts,
        message=f"Campaign started (task: {task_id})",
    )
