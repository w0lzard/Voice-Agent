...
function StatusBadge({ status }) {
  ...
  const cls = map[status?.toLowerCase()] || 'bg-slate-700/50 text-slate-400';
  return (
...
function SentimentBadge({ sentiment }) {
  ...
  return (
    <span className={`material-symbols-outlined fill-1 text-base ${s.cls}`}>{s.icon}</span>
  );
}
...
export default function CallHistoryPage() {
  ...
  const filtered = calls.filter(c => {
    ...
    const matchSearch = !q || (c.lead_phone || c.phone_number || '').toLowerCase().includes(q) ||
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5">
                  {['Date/Time', 'Contact', 'AI Agent', 'Duration', 'Sentiment', 'Status'].map(h => (
              ...
              <tbody>
                {filtered.map((call, i) => {
                  ...
                  const createdAt = call.created_at ? new Date(call.created_at).toLocaleString() : '—';
                  return (
                    <tr key={call.call_id || i} className="border-b border-white/5 hover:bg-white/2 transition-colors">
                      <td className="px-5 py-3 text-xs text-slate-400">{createdAt}</td>
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2">
                          <div className="size-7 rounded-full bg-primary/20 flex items-center justify-center text-primary text-[10px] font-bold">{initials}</div>
                          <span className="text-sm text-slate-300 font-mono text-xs">{phone}</span>
                        </div>
                      </td>
